...
Club::Club(
    const int tables_count,
    const int opening_time,
    const int closing_time,
    const int cost_per_hour
)
...
void Club::add_input_event(const Event &event) {
...
const std::vector<Table> &Club::get_tables() const {
    return tables_;
}

int Club::get_next_sequence() {
...
void Club::handle_client_wait(const Event &event) {
    ...
    if (waiting_queue_.size() >= tables_.size()) {
        ...
        output_events_.emplace_back(
            event.time_, 11, std::vector{client}, get_next_sequence()
        );
        return;
    }
...
void Club::generate_error(
    const int time,
    const std::string &error,
    int trigger_sequence
) {
    const auto it = std::ranges::find_if(
        output_events_, [trigger_sequence](const Event &e
                        ) { return e.sequence_ == trigger_sequence; }
    );

    if (it != output_events_.end()) {
        output_events_.insert(
            it + 1, Event(time, 13, {error}, get_next_sequence())
        );
    }
}
...
void Club::generate_closing_events() {
    ...
    for (const auto &client : remaining_clients) {
        output_events_.emplace_back(
            closing_time_, 11, std::vector{client}, get_next_sequence()
        );
        if (clients_[client] != -1) {
...
void Club::free_table(const int table_index, int end_time) {
    ...
    if (!waiting_queue_.empty()) {
        ...
        output_events_.emplace_back(
            end_time, 12,
            std::vector{new_client, std::to_string(table_index + 1)},
            get_next_sequence()
        );
    }
}
